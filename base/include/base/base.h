...
///
#define BASE_COUNTOF(_x) sizeof(base::CountOfRequireArrayArgumentT(_x) )

///
#define BASE_OFFSETOF(_type, _member) \
	(reinterpret_cast<ptrdiff_t>(&(reinterpret_cast<_type*>(16)->_member) )-ptrdiff_t(16) )

///
#if BASE_COMPILER_MSVC
#	define BASE_IGNORE_C4127(_x) base::ignoreC4127(!!(_x) )
#else
#	define BASE_IGNORE_C4127(_x) (!!(_x) )
#endif // BASE_COMPILER_MSVC

///
#define BASE_ENABLED(_x) BASE_IGNORE_C4127(base::isEnabled<!!(_x)>::value)

///
#define BASE_DECLARE_TAG(_name)  \
	struct    _name ## Tag {}; \
...
{
	/// Placement new tag.
	BASE_DECLARE_TAG(PlacementNew);

	/// Fields are left uninitialized.
	BASE_DECLARE_TAG(InitNone);

	/// Fields are initialized to zero.
	BASE_DECLARE_TAG(InitZero);

	/// Fields are initialized to identity value.
	BASE_DECLARE_TAG(InitIdentity);
