...
#ifndef BASE_RNG_H_HEADER_GUARD
#	error "Must be included from base/rng.h!"
#endif // BASE_RNG_H_HEADER_GUARD
...
namespace base
{
	...
	{
		return 2.0f * base::frnd(_rng) - 1.0f;
	}

	template <typename Rng>
	inline base::Vec3 randUnitCircle(Rng* _rng)
	{
	...
	template <typename Rng>
	inline base::Vec3 randUnitSphere(Rng* _rng)
	{
	...
	inline base::Vec3 randUnitHemisphere(Ty* _rng, const base::Vec3& _normal)
	{
		const base::Vec3 dir = randUnitSphere(_rng);
		const float ddotn  = base::dot(dir, _normal);
	...
	{
		BASE_ASSERT(_num != 0, "Number of elements can't be 0!");

		for (uint32_t ii = 0, num = _num-1; ii < num; ++ii)
		{
			uint32_t jj = ii + 1 + _rng->gen() % (num - ii);
			base::swap(_array[ii], _array[jj]);
		}
	}
