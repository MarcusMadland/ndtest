...
#	error "Must be included from base/spscqueue.h!"
...
namespace base
{
	...
	inline SpScUnboundedQueue::SpScUnboundedQueue(AllocatorI* _allocator)
		...
		, m_first(BASE_NEW(m_allocator, Node)(NULL) )
		, m_divider(m_first)
		, m_last(m_first)
	{
	}
	...
	inline SpScUnboundedQueue::~SpScUnboundedQueue()
	{
		while (NULL != m_first)
		{
			Node* node = m_first;
			m_first = node->m_next;
			base::deleteObject(m_allocator, node);
		}
	}

	inline void SpScUnboundedQueue::push(void* _ptr)
	{
		m_last->m_next = BASE_NEW(m_allocator, Node)(_ptr);
