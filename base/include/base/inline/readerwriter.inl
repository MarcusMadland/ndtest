...
#	error "Must be included from base/readerwriter!"
...
namespace base
{
	...
	inline void* MemoryBlock::more(uint32_t _size)
	{
		...
		{
			m_size += _size;
			m_data = base::realloc(m_allocator, m_data, m_size);
		}
	...
	inline int32_t SizerWriter::write(const void* /*_data*/, int32_t _size, Error* _err)
	{
		...
		if (size != _size)
		{
			BASE_ERROR_SET(_err, kErrorReaderWriterWrite, "SizerWriter: write truncated.");
		}
		return size;
	}
	...
	inline int32_t MemoryReader::read(void* _data, int32_t _size, Error* _err)
	{
		...
		if (size != _size)
		{
			BASE_ERROR_SET(_err, kErrorReaderWriterRead, "MemoryReader: read truncated.");
		}
		return size;
	}
	...
	inline int32_t MemoryWriter::write(const void* _data, int32_t _size, Error* _err)
	{
		...
		if (size != _size)
		{
			BASE_ERROR_SET(_err, kErrorReaderWriterWrite, "MemoryWriter: write truncated.");
		}
		return size;
	}
	...
	{
		BASE_ERROR_SCOPE(_err);
		BASE_STATIC_ASSERT(isTriviallyCopyable<Ty>() );
	...
	{
		BASE_ERROR_SCOPE(_err);
		const int64_t current = base::seek(_reader);
		const int64_t aligned = ( (current + _alignment-1)/_alignment) * _alignment;
		const int32_t size    = int32_t(aligned - current);
		if (0 != size)
		{
			const int64_t offset  = base::seek(_reader, size);
			if (offset != aligned)
			{
				BASE_ERROR_SET(_err, kErrorReaderWriterWrite, "Align: read truncated.");
			}
	...
	{
		BASE_ERROR_SCOPE(_err);
		const int64_t current = base::seek(_writer);
		const int64_t aligned = ( (current + _alignment-1)/_alignment) * _alignment;
		const int32_t size    = int32_t(aligned - current);
		if (0 != size)
		{
	...
	{
		BASE_ERROR_USE_TEMP_WHEN_NULL(_err);
