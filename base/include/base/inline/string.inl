...
#ifndef BASE_STRING_H_HEADER_GUARD
#	error "Must be included from base/string.h!"
#endif // BASE_STRING_H_HEADER_GUARD

#if BASE_CRT_MSVC && !defined(va_copy)
#	define va_copy(_a, _b) (_a) = (_b)
#endif // BASE_CRT_MSVC && !defined(va_copy)
...
namespace base
{
	...
	inline constexpr StringLiteral::StringLiteral(const char (&str)[SizeT])
		...
		, m_len(SizeT - 1)
	{
		BASE_ASSERT('\0' == m_ptr[SizeT - 1], "Must be 0 terminated.");
	}
	...
	template<base::AllocatorI** AllocatorT>
	...
	inline void StringT<AllocatorT>::append(const StringView& _str)
	{
		if (0 != _str.getLength() )
		{
			...
			if (len+1 > m_capacity)
			{
				...
				ptr = (char*)base::realloc(*AllocatorT, 0 != m_capacity ? ptr : NULL, capacity);
	...
	inline void StringT<AllocatorT>::clear()
	{
		...
		if (0 != m_capacity)
		{
			base::free(*AllocatorT, const_cast<char*>(m_ptr) );
	...
	inline LineReader::LineReader(const base::StringView& _str)
		: m_str(_str)
	{
		reset();
	}
