...
#	error "Must be included from base/handlealloc.h!"
...
namespace base
{
	...
	{
		uint8_t* ptr = (uint8_t*)base::alloc(_allocator, sizeof(HandleAlloc) + 2*_maxHandles*sizeof(uint16_t) );
		return BASE_PLACEMENT_NEW(ptr, HandleAlloc)(_maxHandles);
	}

	inline void destroyHandleAlloc(AllocatorI* _allocator, HandleAlloc* _handleAlloc)
	{
		_handleAlloc->~HandleAlloc();
		base::free(_allocator, _handleAlloc);
	}
	...
	{
		BASE_ASSERT(isValid(_handle), "Invalid handle %d!", _handle);
